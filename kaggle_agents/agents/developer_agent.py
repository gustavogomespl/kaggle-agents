    ...
    def _build_dynamic_instructions(
        ...
        if component.component_type == "model":
            ...
            if "optuna" in component.name.lower() or "tuned" in component.name.lower() or "optimized" in component.name.lower():
                ...
                instructions.append("    2. IMPORTANT: Use stratify=y ONLY for classification (y.dtype categorical or y.nunique() < 20)")
                instructions.append("    3. For regression (continuous y), do NOT use stratify parameter")
                instructions.append("    4. Run Optuna study on this 25% sample ONLY")
                ...
                instructions.append("        # Use 3-fold CV on subsample (faster)")
                instructions.append("        return cross_val_score(model, tune_X, tune_y, cv=3, scoring='roc_auc').mean()")
                instructions.append("    ")

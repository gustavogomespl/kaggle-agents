...
"""
    ...
    def _validate_plan(self, plan: List[AblationComponent]) -> List[AblationComponent]:
        ...
        valid_plan = [c for c in plan if c.estimated_impact > 0]

        # CRITICAL: Ensure at least ONE model component exists
        # Model components are required to generate predictions and submission.csv
        has_model = any(c.component_type == "model" for c in valid_plan)

        if not has_model:
            print("  ⚠️  No 'model' component found - adding baseline model")
            # Add a baseline model component
            baseline_model = AblationComponent(
                name="baseline_model",
                component_type="model",
                code="",  # Will be generated by developer agent
                estimated_impact=0.15,  # Higher impact since it generates predictions
                tested=False,
                actual_impact=None,
            )
            valid_plan.append(baseline_model)
            print(f"     Added: {baseline_model.name} (model)")

        # Ensure at least 3 components total
        if len(valid_plan) < 3:

...
def build_stacking_oof_instructions(working_dir: str, component_name: str) -> list[str]:
    ...
    instructions = [
        ...
        "  7b. 🚨🚨🚨 SAVE TEST IDS (CRITICAL - PREVENTS score=0.50 BUG) 🚨🚨🚨:",
        "     - THE #1 CAUSE OF 0.50 RANDOM SCORES: glob() loads test files in WRONG order!",
        "     - CANONICAL_TEST_IDS is AUTO-INJECTED from sample_submission - USE IT!",
        "     - MUST load test files in CANONICAL_TEST_IDS order, NOT glob order:",
        "       ```python",
        "       # ❌ WRONG - causes score=0.50:",
        "       test_files = list(TEST_DIR.glob('*.jpg'))  # Filesystem order = WRONG!",
        "       ",
        "       # ✅ CORRECT - use CANONICAL_TEST_IDS (auto-injected global):",
        "       test_files = [TEST_DIR / f'{tid}.jpg' for tid in CANONICAL_TEST_IDS]",
        "       # OR with extension flexibility:",
        "       test_files = [next(TEST_DIR.glob(f'{tid}.*')) for tid in CANONICAL_TEST_IDS]",
        "       ",
        "       # Generate predictions in this order",
        "       test_preds = []",
        "       for tid, fpath in zip(CANONICAL_TEST_IDS, test_files):",
        "           pred = model.predict(load(fpath))",
        "           test_preds.append(pred)",
        "       ",
        "       # Save predictions and IDs",
        f"       np.save(str(Path('{working_dir}') / 'models' / 'test_ids_{component_name}.npy'), CANONICAL_TEST_IDS)",
        f"       np.save(str(Path('{working_dir}') / 'models' / 'test_{component_name}.npy'), np.array(test_preds))",
        "       ",
        "       # ALWAYS validate before saving submission!",
        "       validate_submission_alignment(CANONICAL_TEST_IDS)  # auto-injected function",
        "       ```",
        "     - WHY: glob() order is filesystem-dependent (alphabetical or random)",
        "     - sample_submission has DIFFERENT order → predictions assigned to WRONG rows",
        "     - This causes RANDOM PREDICTIONS → score=0.50 despite good validation!",

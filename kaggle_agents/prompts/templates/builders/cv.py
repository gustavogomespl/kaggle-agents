...
def build_stacking_oof_instructions(working_dir: str, component_name: str) -> list[str]:
    ...
    instructions = [
        ...
        "  7b. **SAVE TEST IDS (CRITICAL FOR SUBMISSION ALIGNMENT - PREVENTS score=0.50 BUG)**:",
        "     - MUST save test IDs in the SAME ORDER as test predictions:",
        "       ```python",
        "       # Get test IDs from sample_submission (CANONICAL ORDER)",
        "       sample_sub = pd.read_csv(SAMPLE_SUBMISSION_PATH)",
        "       test_ids = sample_sub.iloc[:, 0].values  # First column is ID",
        f"       np.save(str(Path('{working_dir}') / 'models' / 'test_ids_{component_name}.npy'), test_ids)",
        "       print(f'[LOG:INFO] Saved test_ids: {{len(test_ids)}} IDs')",
        "       ```",
        "     - WHY: Without test_ids, ensemble assumes predictions align with sample_submission order",

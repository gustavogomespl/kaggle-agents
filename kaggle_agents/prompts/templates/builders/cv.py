...
def build_stacking_oof_instructions(working_dir: str, component_name: str) -> list[str]:
    ...
    instructions = [
        ...
        "       ```",
        "     - Save class order under this component's immutable identity:",
        f"       `np.save(str(Path('{working_dir}') / 'models' / 'class_order_{component_name}.npy'), np.asarray([str(v) for v in class_order], dtype=str), allow_pickle=False)`",
        "  4. During CV loop:",
        ...
        "         test_ids=test_ids,           # semantic test entity IDs, prediction order",
        "         class_order=class_order,     # multiclass only; omit otherwise",
        "     )",
        "     ```",
        "     It writes oof_/test_/train_ids_/test_ids_ under the correct component",
        "     name and validates row alignment. Do not hand-roll these np.save calls.",
        "     The literal np.save forms below are reference only, for the rare case",
        "     where the helper is unavailable:",
        "  7. **SAVE ALIGNMENT METADATA (CRITICAL FOR ENSEMBLE VALIDATION)**:",
        "     - Train IDs (row order), passed to the helper as train_ids= or left",
        "       to default to CANONICAL_TRAIN_IDS:",
        "       ```python",
        "       raw_train_ids = train_df['id'].values if 'id' in train_df.columns else train_df.index.values",
        "       train_ids = np.asarray([str(v) for v in raw_train_ids], dtype=str)",
        "       ```",
        ...
        "  7b. **SAVE TEST IDS (CRITICAL FOR SUBMISSION ALIGNMENT)**:",
        "     - MUST save one semantic test record ID per prediction row, in the SAME ORDER as test predictions:",
        "       ```python",
        "       if 'TEST_REC_IDS' in globals() and TEST_REC_IDS:",
        "           test_ids = np.asarray([str(v) for v in TEST_REC_IDS], dtype=str)",
        "       else:",
        "           # Derive IDs from the loaded test entities in their prediction order.",
        "           # The sample first column is valid only for one-row-per-entity formats.",
        "           test_ids = np.asarray([str(v) for v in test_entity_ids], dtype=str)",
        "       assert len(test_ids) == test_preds_aligned.shape[0], \\",
        "           'Need exactly one semantic test ID per prediction row'",
        "       # Pass these to save_component_artifacts(test_ids=test_ids)",
        "       ```",
        "     - For long submissions, do NOT save the repeated/encoded submission row IDs as model test IDs",
        "     - WHY: Explicit semantic IDs let the ensemble align predictions before submission expansion",
        "     - If test data was loaded in different order (e.g., glob), predictions will be MISALIGNED",
        "     - A different row order invalidates the submission despite good validation.",
        ...
        "     ",
        "     # 5. Verify class order (multiclass)",

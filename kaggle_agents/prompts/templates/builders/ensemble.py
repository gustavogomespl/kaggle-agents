...
def build_ensemble_instructions(target_col: str = "target") -> list[str]:
    ...
    instructions = [
        ...
        f"  - CRITICAL: use the injected write_submission(final_predictions) helper; target_col='{target_col}' is only context",
        "  - Never infer submission roles from column positions or write submission.csv directly",

...
def build_model_component_instructions(
    ...
    instructions.extend(
        [
            "",
            "🚨🚨🚨 CRITICAL: TEST ID ALIGNMENT (PREVENTS score=0.50 BUG) 🚨🚨🚨",
            "",
            "  THE #1 CAUSE OF 0.50 RANDOM SCORES: Loading test files via glob() in filesystem",
            "  order instead of sample_submission order. Your predictions end up assigned to",
            "  WRONG rows, causing effectively random predictions!",
            "",
            "  ❌ ABSOLUTELY FORBIDDEN (causes score=0.50):",
            "  ```python",
            "  test_files = list(TEST_DIR.glob('*.jpg'))  # WRONG! Filesystem order",
            "  test_ids = [f.stem for f in test_files]    # WRONG! Random order",
            "  for f in test_files:                       # WRONG! Predictions misaligned",
            "      pred = model(load(f))",
            "  sample_sub[col] = preds  # DISASTER! Row i has prediction for wrong file",
            "  ```",
            "",
            "  ✅ MANDATORY PATTERN - USE CANONICAL_TEST_IDS (auto-injected global):",
            "  ```python",
            "  # CANONICAL_TEST_IDS is already loaded from sample_submission.csv",
            "  # Load test files in THIS ORDER, not glob order!",
            "  test_files = [TEST_DIR / f'{tid}.jpg' for tid in CANONICAL_TEST_IDS]",
            "  # OR if extension varies:",
            "  test_files = [next(TEST_DIR.glob(f'{tid}.*')) for tid in CANONICAL_TEST_IDS]",
            "  ",
            "  # Generate predictions in CANONICAL order",
            "  preds = []",
            "  for tid, fpath in zip(CANONICAL_TEST_IDS, test_files):",
            "      pred = model(load(fpath))",
            "      preds.append(pred)",
            "  ",
            "  # Now safe to assign directly (order matches!)",
            "  sample_sub[col] = preds",
            "  ",
            "  # ALWAYS validate before saving:",
            "  validate_submission_alignment(CANONICAL_TEST_IDS)  # auto-injected function",
            "  ```",
            "",
            ...
            "  ```python",
            "  # STEP 1: Use CANONICAL_TEST_IDS (already loaded from sample_submission)",
            "  # This is the ONLY correct order for test predictions!",
            "  sample_sub = pd.read_csv(SAMPLE_SUBMISSION_PATH)",
            "  ID_COL = sample_sub.columns[0]",
            "  ",
            "  # STEP 2: Generate predictions in CANONICAL_TEST_IDS order",
            "  # (Load test data in sample_sub ID order, NOT glob order!)",
            "  ",
            "  # STEP 3: MUST save test_ids alongside predictions for ensemble alignment!",
            "  np.save(MODELS_DIR / f'test_ids_{COMPONENT_NAME}.npy', CANONICAL_TEST_IDS)",
            ...
            "  validate_submission_alignment(CANONICAL_TEST_IDS)  # Use injected function",
            ...
            "",
            "  ✅ ALWAYS use CANONICAL_TEST_IDS for test file iteration order",
            "  ✅ ALWAYS save test_ids_{name}.npy alongside test_{name}.npy",
            "  ✅ ALWAYS call validate_submission_alignment() before saving",
        ]
    )
